...
impl Database for PostgresDB {
    async fn get_user_by_id(&self, id: i32) -> super::DatabaseResult<crate::models::User> {
        ...
        let res = sync_to_async(move || {
            users.find(id).first::<crate::models::User>(&c)
        }).await?;
        Ok(res)
    }

    async fn get_users_by_id_list(&self, id_list: Vec<i32>) -> super::DatabaseResult<Vec<crate::models::User>> {
        let c = self.pool.get()?;
    ...
    async fn get_companies_by_id_list(&self, id_list: Vec<i32>) -> super::DatabaseResult<Vec<crate::models::Company>> {
        todo!()
    }
}
